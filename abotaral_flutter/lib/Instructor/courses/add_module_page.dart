...
class AddModulePage extends StatefulWidget {
  ...
  const AddModulePage({super.key, required this.courseId, required this.courseTitle});
  ...
  Future<void> _createModule() async {
    if (_moduleTitleController.text.isEmpty ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please fill in module title and description'),
    ...
    try {
      ...
      final module = Module(
        ...
        contentBody: _moduleDescriptionController.text,
        orderIndex: 0, // You might need logic to determine order
      );
      ...
      final moduleId = moduleResponse['id'] as String;

      // 2. Create Lessons
      for (int i = 0; i < _lessons.length; i++) {
        final lessonItem = _lessons[i];
        if (lessonItem.titleController.text.isNotEmpty) {
          final lesson = Lesson(
            moduleId: moduleId,
            title: lessonItem.titleController.text,
            contentBody: '', // Placeholder, maybe add field for content
            orderIndex: i,
          );
